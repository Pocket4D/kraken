...
class RenderEditable extends RenderBox with RelayoutWhenSystemFontsChangeMixin {
  ...
  void _handleSelectionChange(
    ...
    final bool focusingEmpty =
        nextSelection.baseOffset == 0 && nextSelection.extentOffset == 0 && !hasFocus;
  ...
  List<TextSelectionPoint> getEndpointsForSelection(TextSelection selection) {
    ...
    final List<ui.TextBox> boxes =
        selection.isCollapsed ? <ui.TextBox>[] : _textPainter.getBoxesForSelection(selection);
    if (boxes.isEmpty) {
  ...
  Rect getLocalRectForCaret(TextPosition caretPosition) {
    ...
    Rect rect =
        Rect.fromLTWH(0.0, 0.0, cursorWidth, cursorHeight).shift(caretOffset + _paintOffset);
  ...
  void selectWordsInRange(
      {@required Offset from, Offset to, @required SelectionChangedCause cause}) {
    assert(cause != null);
    assert(from != null);
    _layoutText(minWidth: constraints.minWidth, maxWidth: constraints.maxWidth);
    if (onSelectionChanged == null) {
  ...
  void _layoutText({double minWidth = 0.0, double maxWidth = double.infinity}) {
    ...
    if (selection != null && !_floatingCursorOn) {
      if (selection.isCollapsed && _showCursor.value && cursorColor != null)
        showCaret = true;
      else if (!selection.isCollapsed && _selectionColor != null) showSelection = true;
    }
    ...
    final bool shouldClipWidth =
        _textOverflow == TextOverflow.ellipsis || _textOverflow == TextOverflow.fade;
    ...
    switch (defaultTargetPlatform) {
      ...
      case TargetPlatform.windows:
        _caretPrototype = Rect.fromLTWH(
            0.0, _kCaretHeightOffset, cursorWidth, cursorHeight - 2.0 * _kCaretHeightOffset);
        break;
    }
  }
  ...
  void setFloatingCursor(
      FloatingCursorDragState state, Offset boundedOffset, TextPosition lastTextPosition,
  ...
  Offset calculateBoundedFloatingCursorOffset(Offset rawCursorOffset) {
    ...
    final double bottomBound =
        _textPainter.height - preferredLineHeight + floatingCursorAddedMargin.bottom;
  ...
  void _paintContents(PaintingContext context, Offset offset) {
    ...
    if (showSelection) {
      ...
      _selectionRects ??= _textPainter.getBoxesForSelection(selection,
          boxHeightStyle: _selectionHeightStyle, boxWidthStyle: _selectionWidthStyle);
