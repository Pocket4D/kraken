...
typedef Native_InvokeEventListener = Void Function(Int32 contextId, Pointer<NativeString>,
    Pointer<Utf8> eventType, Pointer<Void> nativeEvent, Pointer<NativeString>);
typedef Dart_InvokeEventListener = void Function(int contextId, Pointer<NativeString>,
    Pointer<Utf8> eventType, Pointer<Void> nativeEvent, Pointer<NativeString>);

final Dart_InvokeEventListener _invokeModuleEvent = nativeDynamicLibrary
    .lookup<NativeFunction<Native_InvokeEventListener>>('invokeModuleEvent')
    .asFunction();
...
final Dart_InitJSContextPool _initJSContextPool = nativeDynamicLibrary
    .lookup<NativeFunction<Native_InitJSContextPool>>('initJSContextPool')
    .asFunction();
...
final Dart_DisposeContext _disposeContext = nativeDynamicLibrary
    .lookup<NativeFunction<Native_DisposeContext>>('disposeContext')
    .asFunction();
...
final Dart_AllocateNewContext _allocateNewContext = nativeDynamicLibrary
    .lookup<NativeFunction<Native_AllocateNewContext>>('allocateNewContext')
    .asFunction();
...
final Dart_FlushBridgeTask _flushBridgeTask = nativeDynamicLibrary
    .lookup<NativeFunction<Native_FlushBridgeTask>>('flushBridgeTask')
    .asFunction();
...
final Dart_FlushUICommandCallback _flushUICommandCallback = nativeDynamicLibrary
    .lookup<NativeFunction<Native_FlushUICommandCallback>>('flushUICommandCallback')
    .asFunction();
...
final Dart_GetUICommandItems _getUICommandItems = nativeDynamicLibrary
    .lookup<NativeFunction<Native_GetUICommandItems>>('getUICommandItems')
    .asFunction();
...
final Dart_GetUICommandItemSize _getUICommandItemSize = nativeDynamicLibrary
    .lookup<NativeFunction<Native_GetUICommandItemSize>>('getUICommandItemSize')
    .asFunction();
...
final Dart_ClearUICommandItems _clearUICommandItems = nativeDynamicLibrary
    .lookup<NativeFunction<Native_ClearUICommandItems>>('clearUICommandItems')
    .asFunction();
...
List<UICommand> readNativeUICommandToDart(
  ...
  List<UICommand> results = []..length = commandLength;
  ...
  for (int i = 0; i < commandLength * nativeCommandSize; i += nativeCommandSize) {
    ...
    command.nativePtr =
        nativePtrValue != 0 ? Pointer.fromAddress(rawMemory[i + nativePtrMemOffset]) : nullptr;
...
void flushUICommand() {
  ...
  for (KrakenController controller in controllerMap.values) {
    ...
    List<UICommand> commands =
        readNativeUICommandToDart(nativeCommandItems, commandLength, controller.view.contextId);
