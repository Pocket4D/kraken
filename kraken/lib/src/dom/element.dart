...
class Element extends Node
  ...
  bool get isValidSticky =>
      style[POSITION] == STICKY && (style.contains(TOP) || style.contains(BOTTOM));
  ...
  Element(int targetId, this.nativeElementPtr, ElementManager elementManager,
      ...
      : assert(targetId != null),
        ...
        super(NodeType.ELEMENT_NODE, targetId, nativeElementPtr.ref.nativeNode, elementManager,
            tagName) {
  ...
  void layoutStickyChild(Element child, double scrollOffset, AxisDirection axisDirection) {
    ...
    if (child.originalScrollContainerOffset == null) {
      Offset horizontalScrollContainerOffset = childRenderBoxModel.localToGlobal(Offset.zero,
              ancestor: child.elementManager.getRootRenderObject()) -
          renderBoxModel.localToGlobal(Offset.zero,
              ancestor: child.elementManager.getRootRenderObject());
      Offset verticalScrollContainerOffset = childRenderBoxModel.localToGlobal(Offset.zero,
              ancestor: child.elementManager.getRootRenderObject()) -
          renderBoxModel.localToGlobal(Offset.zero,
              ancestor: child.elementManager.getRootRenderObject());
  ...
  void toggleRepaintSelf({bool repaintSelf}) {
    ...
    RenderBoxModel targetRenderBox =
        createRenderBoxModel(this, prevRenderBoxModel: renderBoxModel, repaintSelf: repaintSelf);
    ...
    switch (positionType) {
      ...
      case CSSPositionType.static:
        RenderLayoutBox parentRenderLayoutBox =
            scrollingContentLayoutBox != null ? scrollingContentLayoutBox : _renderLayoutBox;
  ...
  void attachTo(Element parent, {RenderObject after}) {
    ...
    // InlineFlex or Flex
    bool isParentFlexDisplayType =
        parentDisplayValue == CSSDisplay.flex || parentDisplayValue == CSSDisplay.inlineFlex;
    ...
    if (isRendererAttached) {
      ...
      if (!child.isRendererAttached) {
        ...
        if (referenceIndex != -1 && referenceNode.isRendererAttached) {
          afterRenderObject =
              (referenceNode.renderer.parentData as ContainerBoxParentData).previousSibling;
        }
  ...
  void _addStickyChild(Element child, RenderObject after) {
    RenderBoxModel childRenderBoxModel = child.renderBoxModel;
    RenderLayoutBox parentRenderLayoutBox =
        scrollingContentLayoutBox != null ? scrollingContentLayoutBox : renderBoxModel;
  ...
  BoundingClientRect get boundingClientRect {
    ...
    if (isRendererAttached) {
      ...
      boundingClientRect = BoundingClientRect(offset.dx, offset.dy, size.width, size.height,
          offset.dy, offset.dx + size.width, offset.dy + size.height, offset.dx);
    }
  ...
  void addEvent(String eventType) {
    ...
    bool hasIntersectionObserverEvent =
        isIntersectionObserverEvent && _hasIntersectionObserverEvent(eventHandlers);
  ...
  void removeEvent(String eventType) {
    ...
    if (renderBoxModel != null) {
      ...
      if (_isIntersectionObserverEvent(eventType) &&
  ...
  void _eventResponder(Event event) {
    emitUIEvent(elementManager.controller.view.contextId,
        nativeElementPtr.ref.nativeNode.ref.nativeEventTarget, event);
  }
  ...
  void handleMethodClick() {
    ...
    if (isRendererAttached) {
      ...
      Offset position = box.localToGlobal(box.size.center(Offset.zero),
          ancestor: elementManager.getRootRenderObject());
  ...
  Future<Uint8List> toBlob({double devicePixelRatio}) {
    ...
    if (!renderBoxModel.isRepaintBoundary) {
      ...
      if (renderBoxModel is RenderLayoutBox) {
        renderReplacedBoxModel =
            createRenderLayout(this, prevRenderLayoutBox: renderBoxModel, repaintSelf: true);
      } else {
        renderReplacedBoxModel =
            createRenderIntrinsic(this, prevRenderIntrinsic: renderBoxModel, repaintSelf: true);
      }
  ...
  RenderBoxModel createRenderBoxModel(Element element,
      {RenderBoxModel prevRenderBoxModel, bool repaintSelf = false}) {
    RenderBoxModel renderBoxModel = prevRenderBoxModel ?? element.renderBoxModel;

    if (renderBoxModel is RenderIntrinsic) {
      return createRenderIntrinsic(element,
          prevRenderIntrinsic: prevRenderBoxModel, repaintSelf: repaintSelf);
    } else {
      return createRenderLayout(element,
          prevRenderLayoutBox: prevRenderBoxModel, repaintSelf: repaintSelf);
    }
  }

  RenderLayoutBox createRenderLayout(Element element,
    ...
    CSSDisplay display = CSSDisplayMixin.getDisplay(
        CSSStyleDeclaration.isNullOrEmptyValue(style[DISPLAY])
  ...
  RenderIntrinsic createRenderIntrinsic(Element element,
    ...
    if (prevRenderIntrinsic == null) {
      if (repaintSelf) {
        intrinsic =
            RenderSelfRepaintIntrinsic(element.targetId, renderStyle, element.elementManager);
      } else {
...
bool _isIntersectionObserverEvent(String eventType) {
  return eventType == EVENT_APPEAR ||
      eventType == EVENT_DISAPPEAR ||
      eventType == EVENT_INTERSECTION_CHANGE;
}
...
class BoundingClientRect {
  ...
  BoundingClientRect(
      this.x, this.y, this.width, this.height, this.top, this.right, this.bottom, this.left);

  Pointer<NativeBoundingClientRect> toNative() {
    Pointer<NativeBoundingClientRect> nativeBoundingClientRect = calloc<NativeBoundingClientRect>();
...
void _setPositionedChildParentData(RenderLayoutBox parentRenderLayoutBox, Element child) {
  ...
  RenderBoxModel childRenderBoxModel = child.renderBoxModel;
  childRenderBoxModel.parentData =
      CSSPositionedLayout.getPositionParentData(childRenderBoxModel, parentData);
}
