...
program
  .option(
    "-e, --js-engine <engine>",
    "The JavaScript Engine kraken used",
    "jsc"
  )
  .parse(process.argv);
...
const pkgVersion = readFileSync(
  path.join(paths.kraken, "pubspec.yaml"),
  "utf-8"
)
  .match(/version: (.*)/)[1]
  .trim();
...
if (platform == "win32") {
  ...
  if (!fs.existsSync(winShell)) {
    return done(
      new Error(
        `Can not location bash.exe, Please install Git for Windows at ${process.env.ProgramW6432}. \n https://git-scm.com/download/win`
      )
    );
  }
}
...
task("copy-kraken-debug", (done) => {
  ...
  if (platform === "darwin") {
    ...
    execSync(
      `mv ${path.join(
        paths.example,
        "build/macos/Build/Products/Debug/kraken_example.app"
      )} ${targetDist}`
    );
    return done();
  }
  ...
  if (platform === "linux") {
    execSync(
      `cp -r ${path.join(paths.example, "build/linux/debug/*")} ${targetDist}`
    );
    return done();
  }
...
task("copy-kraken-release", (done) => {
  ...
  if (platform === "darwin") {
    execSync(
      `mv ${path.join(
        paths.example,
        "build/macos/Build/Products/Release/kraken_example.app"
      )} ${targetDist}`
    );
    ...
    writeFileSync(
      join(
        targetDist,
        "kraken_example.app/Contents/Frameworks/App.framework/Resources/flutter_assets/.keep"
      ),
      "# Just keep it."
    );
    return done();
  ...
  if (platform === "linux") {
    execSync(
      `mv ${path.join(paths.example, "build/linux/release/")} ${targetDist}`
    );
    return done();
  }
...
task("build-darwin-kraken-lib", (done) => {
  ...
  execSync(
    `cmake -DCMAKE_BUILD_TYPE=${buildType} -DENABLE_TEST=true ${
      isProfile ? "-DENABLE_PROFILE=TRUE" : ""
    } \
    -G "Unix Makefiles" -B ${paths.bridge}/cmake-build-macos-x86_64 -S ${
      paths.bridge
    }`,
    {
      cwd: paths.bridge,
  ...
  execSync(
    `cmake --build ${paths.bridge}/cmake-build-macos-x86_64 --target kraken kraken_test -- -j 12`,
    {
      stdio: "inherit",
    }
  );
  ...
  const binaryPath = path.join(
    paths.bridge,
    `build/macos/lib/x86_64/libkraken_${program.jsEngine}.dylib`
  );
...
task("compile-polyfill", (done) => {
  ...
  let result = spawnSync(
    "npm",
    ["run", buildMode === "Release" ? "build:release" : "build"],
    {
      cwd: paths.polyfill,
      env: {
        ...process.env,
...
task(`build-ios-kraken-lib`, (done) => {
  ...
  execSync(
    ...
    ${isProfile ? "-DENABLE_PROFILE=TRUE \\" : "\\"}
    -DENABLE_BITCODE=FALSE -G "Unix Makefiles" -B ${
      paths.bridge
    }/cmake-build-ios-x64 -S ${paths.bridge}`,
    {
      cwd: paths.bridge,
      stdio: "inherit",
      env: {
  ...
  // build for simulator
  execSync(
    `cmake --build ${paths.bridge}/cmake-build-ios-x64 --target kraken kraken_static -- -j 12`,
    {
      stdio: "inherit",
    }
  );
  ...
  execSync(
    ...
    -DENABLE_BITCODE=FALSE -G "Unix Makefiles" -B ${
      paths.bridge
    }/cmake-build-ios-arm -S ${paths.bridge}`,
    {
      cwd: paths.bridge,
      stdio: "inherit",
      env: {
  ...
  execSync(
    `cmake --build ${paths.bridge}/cmake-build-ios-arm --target kraken kraken_static -- -j 12`,
    {
      stdio: "inherit",
    }
  );
  ...
  execSync(
    `cmake -DCMAKE_BUILD_TYPE=${buildType} \
     ...
     -DENABLE_BITCODE=FALSE -G "Unix Makefiles" -B ${
       paths.bridge
     }/cmake-build-ios-arm64 -S ${paths.bridge}`,
    {
      cwd: paths.bridge,
      stdio: "inherit",
      env: {
  ...
  // build for ARMV64
  execSync(
    `cmake --build ${paths.bridge}/cmake-build-ios-arm64 --target kraken kraken_static -- -j 12`,
    {
      stdio: "inherit",
    }
  );

  const armDynamicSDKPath = path.join(
    paths.bridge,
    "build/ios/lib/arm/kraken_bridge.framework/kraken_bridge"
  );
  const arm64DynamicSDKPath = path.join(
    paths.bridge,
    "build/ios/lib/arm64/kraken_bridge.framework/kraken_bridge"
  );
  const x64DynamicSDKPath = path.join(
    paths.bridge,
    "build/ios/lib/x86_64/kraken_bridge.framework/kraken_bridge"
  );
  ...
  execSync(
    `lipo -create ${armDynamicSDKPath} ${x64DynamicSDKPath} ${arm64DynamicSDKPath} -output ${frameworkPath}/kraken_bridge`,
    {
      stdio: "inherit",
    }
  );
  ...
  if (buildMode == "Release") {
    execSync(`dsymutil ${frameworkPath}/kraken_bridge`, {
      stdio: "inherit",
      cwd: targetDynamicSDKPath,
    });
    execSync(`mv ${frameworkPath}/kraken_bridge.dSYM ${targetDynamicSDKPath}`);
    execSync(`strip -S -X -x ${frameworkPath}/kraken_bridge`, {
      stdio: "inherit",
      cwd: targetDynamicSDKPath,
    });
  }

  const armStaticSDKPath = path.join(
    paths.bridge,
    `build/ios/lib/arm/libkraken_${program.jsEngine}.a`
  );
  const arm64StaticSDKPath = path.join(
    paths.bridge,
    `build/ios/lib/arm64/libkraken_${program.jsEngine}.a`
  );
  const x64StaticSDKPath = path.join(
    paths.bridge,
    `build/ios/lib/x86_64/libkraken_${program.jsEngine}.a`
  );

  const targetStaticSDKPath = `${paths.bridge}/build/ios/framework`;
  execSync(
    `libtool -static -o ${targetStaticSDKPath}/libkraken_${program.jsEngine}.a ${armStaticSDKPath} ${arm64StaticSDKPath} ${x64StaticSDKPath}`
  );
  done();
});
...
task("build-ios-frameworks", (done) => {
  ...
  execSync(
    `cp -r ${paths.bridge}/build/ios/framework/kraken_bridge.framework ${paths.sdk}/build/ios/framework/Debug`
  );
  execSync(
    `cp -r ${paths.bridge}/build/ios/framework/kraken_bridge.dSYM ${paths.sdk}/build/ios/framework/Debug`
  );
  execSync(
    `cp -r ${paths.bridge}/build/ios/framework/kraken_bridge.framework ${paths.sdk}/build/ios/framework/Profile`
  );
  execSync(
    `cp -r ${paths.bridge}/build/ios/framework/kraken_bridge.framework ${paths.sdk}/build/ios/framework/Release`
  );
...
task("build-android-kraken-lib", (done) => {
  ...
  const archs = ["arm64-v8a", "armeabi-v7a", "x86_64", "x86"];
  const buildType = buildMode == "Release" ? "Relwithdebinfo" : "Debug";

  const cmakeGeneratorTemplate =
    platform == "win32" ? "Ninja" : "Unix Makefiles";
  archs.forEach((arch) => {
    const soBinaryDirectory = path.join(
      paths.bridge,
      `build/android/lib/${arch}`
    );
    const bridgeCmakeDir = path.join(
      paths.bridge,
...
task("symbol-link-for-android", (done) => {
  ...
  for (let arch of archs) {
    ...
    fs.copyFileSync(
      path.join(
        paths.thirdParty,
        `JavaScriptCore/lib/android/${arch}/libjsc.so`
      ),
      path.join(libPath, "libjsc.so")
    );
    fs.copyFileSync(
      path.join(
        paths.thirdParty,
        `JavaScriptCore/lib/android/${arch}/libc++_shared.so`
      ),
      path.join(libPath, "libc++_shared.so")
    );
    fs.copyFileSync(
      path.join(paths.bridge, `build/android/lib/${arch}/libkraken_jsc.so`),
      path.join(libPath, "libkraken_jsc.so")
    );
  }
...
task("android-so-clean", (done) => {
  execSync(`rm -rf ${paths.bridge}/build/android`, {
    stdio: "inherit",
    shell: winShell,
  });
  done();
